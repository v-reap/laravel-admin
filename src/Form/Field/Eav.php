...
class Eav extends Field
{
    ...
    public function getValidator(array $input)
    {
        ...
        $form = $this->buildEavForm($this->column, $this->builder);
        ...
        array_forget($rules, EavForm::REMOVE_FLAG_NAME);
     ...
     * @return EavForm
     */
    protected function buildEavForm($column, \Closure $builder, $key = null, $attribute = null)
    {
        $form = new Form\EavForm($column, $key);
        $form->setForm($this->form);

        call_user_func($builder, $form);

        $modelId = $this->form->model()->id;
    ...
    protected function buildRelatedForms()
    {
        ...
        $values = array_column($this->value,'task_value','attribute_id');
        $datas = [];
        foreach($this->value as $key=>$value){
            $datas[$value['attribute_id']]=$value;
        }

        foreach ($this->getAttribites()->toArray() as $attribute) {
            $data=isset($datas[$attribute['id']]) ? $datas[$attribute['id']] : null;
            $key = array_get($data, $relation->getRelated()->getKeyName());

            $forms[$key] = $this->buildEavForm($this->column, $this->builder, $key, $attribute);
            if ($data){
                $forms[$key]->fill($data);
            }
        }
...
//                if ($data[EavForm::REMOVE_FLAG_NAME] == 1) {
//                    continue;
//                }
//
//                $forms[$key] = $this->buildEavForm($this->column, $this->builder, $key)
//                    ->fill($data);
//            }
//        } else {
...
//                $key = array_get($data, $relation->getRelated()->getKeyName());
//
//                $forms[$key] = $this->buildEavForm($this->column, $this->builder, $key)->fill($data);
//            }
//        }
    ...
    public function getAttribites()
    {
        if (!$this->atts){
            $this->atts = \Encore\Admin\Models\Task\Attribute::all()
                ->where('type_id','=',$this->form->model()->type_id);
        }
        return $this->atts;
    }
    ...
    public function render()
    {
        ...
        list($template, $script) = $this->buildEavForm($this->column, $this->builder)
